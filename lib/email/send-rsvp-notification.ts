...
export async function sendRsvpNotification(
  ...
  const from =
    ...
    `${siteConfig.couple.bride} & ${siteConfig.couple.groom} <onboarding@resend.dev>`;

  const dietary = data.dietaryRestrictions || "Belirtilmedi";

  const { error } = await resend.emails.send({
    from,
    to: recipients,
    subject: `Yeni katılım bildirimi — ${data.name}`,
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; color: #373737;">
        ...
        <table style="width: 100%; border-collapse: collapse;">
          <tr>
            ...
            <td style="padding: 12px 0; border-bottom: 1px solid #c3cab8; text-align: right;">${escapeHtml(data.name)}</td>
          </tr>
          <tr>
            ...
            <td style="padding: 12px 0; border-bottom: 1px solid #c3cab8; text-align: right;">${data.guestCount} kişi</td>
          </tr>
          <tr>
